(function(){
 ...
 function options(kind,width,load,selected,manual){const rows=choices(kind,width,load),all=manual?catalog(kind).map(r=>rows.find(x=>x.value===r.value)||r):rows;return '<option value="">'+(rows.length?'Ürün seçin':'Tabloda öneri yok')+'</option>'+all.map(r=>'<option value="'+esc(r.value)+'"'+(r.value===selected?' selected':'')+'>'+esc(label(r))+(rows.some(x=>x.value===r.value)?' · Önerilen':manual?' · Tablo önerisi dışında':'')+'</option>').join('');}
