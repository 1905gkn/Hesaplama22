...
export function groupRackPlan(plan){
  ...
  for(const p of plan.placements){
    ...
    // Pair the matching bays, not whole columns: a narrow bay or unmatched tail
    // must not turn the compatible parts of both columns into single rows.
    const partner=matches.find(q=>q.row!==p.row&&q.depth===p.depth),gap=partner?(Number.isFinite(plan.doubleRowGap)?plan.doubleRowGap:Math.round((Math.abs(partner.x-p.x)-spec.frameDepth)/5)*5):0;
