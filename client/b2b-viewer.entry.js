...
class B2BViewer {
  ...
  addDimensionLabelAt(layer, x, y, z, text, width = 800) {
    ...
    sprite.onBeforeRender = () => {
      ...
      context.fillStyle = 'rgba(5,40,72,.98)'; context.beginPath(); context.roundRect(0, 0, labelWidth, labelHeight, 18); context.fill(); context.stroke();
      context.fillStyle = '#fff'; context.fillText(window.rafexDimensionText?.(text, language) || text, labelWidth / 2, labelHeight / 2, labelWidth - 36);
...
async function captureB2BViews(options = {}, settings = {}) {
  options = { ...options, language: settings.language || options.language || document.getElementById('m2ReportLanguage')?.value || document.documentElement.lang };
