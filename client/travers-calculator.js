(function () {
  ...
  function mount() {
    ...
    form.addEventListener('submit', event => {
      ...
      result.innerHTML = `<h3 class="trv-result-title">${length.toLocaleString('tr-TR')} mm · ${load.toLocaleString('tr-TR')} kg kat yükü</h3><div class="trv-options">${rows.map(row => `<article class="trv-option${row.manual ? ' trv-manual' : ''}"><h4>${escape(row.group)}</h4><p class="trv-section">${row.section ? escape(row.section.replace(/ ST(?:37|52)$/, '').replace(/^Kutu50\.(\d+)\.(.+)$/, 'Kutu $1 × 50 × $2').replace(/^CC(\d+)x50x(.+)$/, 'CC $1 × 50 × $2'))+' mm' : 'Uygun kesit bulunamadı'}</p><p class="trv-status">${row.manual ? escape(row.note) + '. Hesap kontrolünü geçmiyor.' : row.section ? 'Tablodaki öneri' : 'Bu tip ve kalitede tablo kapsamında uygun ürün yok.'}</p></article>`).join('')}</div>`;
    });
  }
