(function () {
  ...
  function mount() {
    ...
    miniSection.querySelectorAll('.trv-scope')[1].textContent = '800–3600 mm ve 200–1500 kg, 100 birim aralıklarla. Önce 1,50 mm, ardından düşük ağırlık. Kaynak hesap koşulları korunmuştur. Yük iki traverse eşit bölünür; hareketli ve deprem yükleri dahil değildir.';
    ...
    form.addEventListener('submit', event => {
      ...
      result.innerHTML = `<p class="trv-scope">Girilen: ${length} mm / ${load} kg</p><h3 class="trv-result-title">Hesapta kullanılan: ${rows[0].length.toLocaleString('tr-TR')} mm · ${rows[0].load.toLocaleString('tr-TR')} kg kat yükü</h3><div class="trv-options">${rows.map(row => `<article class="trv-option${row.manual ? ' trv-manual' : ''}"><h4>${escape(row.group)}</h4><p class="trv-section">${row.section ? escape(row.section.replace(/ ST(?:37|52)$/, '').replace(/^Kutu50\.(\d+)\.(.+)$/, 'Kutu $1 × 50 × $2').replace(/^CC(\d+)x50x(.+)$/, 'CC $1 × 50 × $2').replace(/^ZS30\.(\d+)\.(.+)$/, 'ZS $1 × 30 × $2'))+' mm' : 'Uygun kesit bulunamadı'}</p><p class="trv-status">${row.manual ? 'Yönetici tarafından kabul edildi <span class="trv-warning" tabindex="0" role="img" aria-label="Manuel seçim; hesap sınırını aşıyor" title="Manuel seçim; hesap sınırını aşıyor">!</span>' : row.section ? 'Tablodaki öneri' : 'Bu tip ve kalitede tablo kapsamında uygun ürün yok.'}</p></article>`).join('')}</div>`;
    });
    }
  }
