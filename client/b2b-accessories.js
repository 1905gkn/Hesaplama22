(() => {
  ...
  window.rafexCollectionSet=(index,key,value)=>{const f=collection.floors[index]||freshCollectionFloor();if(key==='groundGap')collection.groundGap=Math.max(0,Math.min(5000,Number(value)||0));else if(key==='trayWidth')f.trayWidth=[200,250,300].includes(Number(value))?Number(value):300;else if(key==='trayThickness')f.trayThickness=[.6,.8,1,1.2,1.5].includes(Number(value))?Number(value):.8;else if(key==='traverse'&&ZS_HEIGHTS[value])f.traverse=value;else if(key==='height')f.height=Math.max(100,Math.min(5000,Number(value)||500));collection.floors[index]=f;render();notify();};
