(() => {
  ...
  function renderCollection() {
    ...
    host.innerHTML=`<div class="b2b-accessory-card-head"><b>1. Toplama Katı</b><button class="b2b-accessory-remove" type="button" onclick="rafexCollectionRemove()">Kaldır</button></div><div class="b2b-collection-form"><label class="b2b-collection-row"><span>Z – 1. kat arası mesafe</span><input aria-label="Z – 1. kat arası mesafe" type="number" min="0" max="5000" step="10" value="${collection.groundGap}" onchange="rafexCollectionSet('groundGap',this.value)"></label><label class="b2b-collection-row"><span>Tava seçimi</span><select aria-label="Tava seçimi" onchange="rafexCollectionSet('trayWidth',this.value)">${opts([300,250,200],f.trayWidth,(v)=>v+' mm')}</select></label><label class="b2b-collection-row"><span>Kalınlık seçimi</span><select aria-label="Kalınlık seçimi" onchange="rafexCollectionSet('trayThickness',this.value)">${opts([.6,.8,1,1.2,1.5],f.trayThickness,(v)=>String(v).replace('.',',')+' mm')}</select></label><label class="b2b-collection-row"><span>ZS travers seçimi</span><select aria-label="ZS travers seçimi" onchange="rafexCollectionSet('traverse',this.value)">${opts(Object.keys(ZS_HEIGHTS),f.traverse,(v)=>v.replace('|',' · ')+' mm')}</select></label><label class="b2b-collection-row"><span>Kat yüksekliği</span><input aria-label="Kat yüksekliği" type="number" min="100" max="5000" step="10" value="${f.height}" onchange="rafexCollectionSet('height',this.value)"></label></div><div class="b2b-accessory-note"><b>ZS toplama yüksekliği · ${p.totalHeight.toLocaleString('tr-TR')} mm</b>${collection.groundGap} mm zemin mesafesi + ${ZS_HEIGHTS[f.traverse]} mm ZS travers + ${f.height} mm kat yüksekliği. Sonrasında CC traversli normal katlar başlar.</div>`;
  }
