(function(){
  ...
  function open(){
    if(!dialog){
      ...
      dialog=document.createElement('dialog');dialog.id='rafexPdfAutoDialog';dialog.innerHTML='<header><h2>PDF / Fotoğraf’den otomatik yerleşim</h2><button data-close aria-label="Kapat">×</button></header><p>Ölçülü B2B plan PDF’sini veya PNG/JPEG görselini seç. Raf tipleri ve konumlar otomatik okunur; ayak ve traversler uygulamanın yük tablolarından hesaplanır.</p><label>PDF / Fotoğraf dosyaları (en fazla 10 dosya, dosya başına 20 MB)<input data-file multiple type="file" accept="application/pdf,image/png,image/jpeg,.pdf,.png,.jpg,.jpeg"></label><p><small>İlk analiz bu tarayıcıda yapılır. Agent ile oku düğmesine basarsanız seçtiğiniz sayfa/bölge OpenAI’ye gönderilir. Vektörel PDF doğrudan okunur. Fotoğraf ve resim PDF’de renkli yatay raf planı ve OCR kullanılır; belirsiz ölçüler sorulur.</small></p><div role="status" aria-live="polite">PDF veya fotoğraf seçerek başlayabilirsin.</div><div data-files></div><div data-raster></div><div data-preview hidden></div><label data-confirm hidden><input type="checkbox"> Uyarıları gördüm. Çakışan gözleri bekleterek mevcut çizimin yerine uygula.</label><footer><button data-close>Vazgeç</button><button data-apply disabled>Raf tiplerini oluştur ve yerleştir</button></footer>';document.body.append(dialog);
  ...
  async function processNext(){
    ...
    if(!item){if(batch.length&&batch.every(i=>i.result)){try{fileName=batch.map(i=>i.name).join(', ');prepare(combinePlans(batch));}catch(e){status(e.message);}}else status(batch.length?'Okunamayan dosyaları tekrar dene veya kaldır.':'PDF veya fotoğraf seçerek başlayabilirsin.');return;}
