...
Ölçü çiftli planlar: `1850/1100`, `2400/1100`, `3300/1100` gibi tekrarlanan açıklık/derinlik etiketleri renkli yatay raf sınırlarıyla eşleştirilir. Sayısal yük tablosunun mm birimli sütunları, kesitte tekrar eden yük tipi kimlikleri ve kg birimli yük kullanılır; Fransızca/İngilizce başlık adı aranmaz. Latin, Kiril, Arapça, Çince, Japonca, Korece ve Devanagari mm yazımları; Batı/Arap/Fars rakamları normalize edilir. Farklı palet tabanları, belirsiz yük birimleri, eşleşmeyen kesitler ve kapasite uyuşmazlığı otomatik uygulanmaz. Bu yapısal okuyucu her dildeki serbest metni anlayan genel bir dil modeli değildir; raster OCR hâlâ İngilizce/sayı modelidir.

Kat bazında `palletHeights` ve travers üstleri arası `levelPitches` korunur. Pitch ölçüleri net açıklık olarak yorumlanmaz. Özel genişlikler `b2b.importedSectionWidth` ile native blok ve görünümde saklanır; travers seçimi gerçek açıklık üzerinden yapılır. PLAN SV-AR testinde 266 göz, 12 sıra, 156 blok, 5488 palet; altı kat 1300 ve üst kat 1700 mm, ayak 10006 mm, kat adımı 1550 mm doğrulandı. Belge adı veya sabit sayfa koordinatı üretim mantığında kullanılmaz.

`RAFEX_DIMENSIONED_FIXTURE` ile `node scripts/verify-pdf-dimensioned-v193.mjs`: gerçek PDF, birim yazımları, Arap rakamları, sayfa kaydırma, eksik yük ve kapasite uyuşmazlığı testleri.
