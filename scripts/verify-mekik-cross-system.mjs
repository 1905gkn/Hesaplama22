...
try{
 ...
 for(const system of ['b2b','mekik2','drive','mr','konsol']){
  ...
  const result=await page.evaluate(({mekik,system})=>{
   const d=structuredClone(mekik);d.rafexSystem='mekik2';
   window.rafexProjectTypesV133=window.rafexMergeRackCatalog([],[{id:321,name:'E',drawing:d,__rafexSnapshot:d,__rafexSystem:'mekik2',__rafexSystemLabel:'Mekik'}]).entries;
   window.rafexUnifiedCatalogSync();m2RenderSavedRackTypes();m2SelectedSavedType=0;
   m2LayoutState.racks=[];m2LayoutState.closed=true;m2LayoutState.points=[{x:0,y:0},{x:2000,y:0},{x:2000,y:1200},{x:0,y:1200}];window.rafexSelectedCatalogKey="mekik2:321";m2AddSelectedSavedRack();m2RenderLayout();
  ...
  },{mekik,system});assert(result.rack,system+' must add a rack');assert.equal(result.rack.rafexSystem,'mekik2');assert.equal(result.rack.layoutView,null,system+' must not attach foreign layout');assert.equal(result.rack.konsol,undefined);assert.equal(result.rack.width,mekik.totalWidth);assert.equal(result.rack.depth,mekik.railLength);await page.waitForTimeout(100);assert.equal(await page.locator('#m2LayoutSvg [data-rafex-konsol-plan]').count(),0);assert.equal(await page.locator('#m2LayoutSvg .rafex-plan-pallet-v143').count(),mekik.bays*mekik.depth);console.log('PASS Mekik placement with active '+system);
 }
