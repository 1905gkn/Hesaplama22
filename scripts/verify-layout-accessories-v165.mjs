...
import {transform as customizePerformance} from './patch-customize-performance-v166.mjs';
const {chromium}=createRequire(import.meta.url)(process.env.RAFEX_PLAYWRIGHT_PATH||'playwright');
const input=fs.readFileSync(process.argv[2]||'outputs/live-after-v165.html','utf8'),html=process.argv.includes('--live')?input:customizePerformance(transform(input));
...
try{
 ...
 await page.evaluate(()=>{
  window.previewTest={updates:0,destroys:0,mounts:0};
  window.RafexB2BViewer={destroy(){},mount(){},update(){},createDetached(canvas,options){previewTest.mounts++;return {setView(){},update(o){previewTest.updates++;previewTest.options=o},destroy(){previewTest.destroys++}}}};
  m2SelectedSymbolId=null;m2MultiSelect.symbolIds.clear();m2LayoutSymbols=[];m2OpenCustomizeModal(m2LayoutState.racks[0].id);
 });
 await page.locator('#m2CustomizeModal').waitFor({state:'visible'});
 await page.waitForFunction(()=>previewTest.mounts===1);
 await page.waitForTimeout(350);
 await page.evaluate(()=>{previewTest.updates=0});
 await page.locator('#m2CustomizeBlockName').fill('Performance test');
 await page.waitForTimeout(250);
 assert.equal(await page.evaluate(()=>previewTest.updates),0,'Name must not rebuild 3D');
 await page.evaluate(()=>{const n=document.getElementById('m2CustomizePalletHeight');for(let i=0;i<100;i++){n.value=String(1700+i);n.dispatchEvent(new Event('input',{bubbles:true}))}});
 await page.waitForFunction(()=>previewTest.updates===1);
 await page.waitForTimeout(250);
 assert.equal(await page.evaluate(()=>previewTest.updates),1,'100 events should rebuild once');
 await page.locator('#m2CustomizePalletHeight').fill('1800');
 await page.evaluate(()=>m2ApplyRackCustomization());
 await page.waitForTimeout(250);
 assert.equal(await page.evaluate(()=>previewTest.destroys),1,'Close destroys detached viewer');
 assert.equal(await page.evaluate(()=>previewTest.updates),1,'Close cancels pending preview');
