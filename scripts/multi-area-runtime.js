(function(){
  ...
  const sessions=new Map();let documentState=null,owner=null,restoring=false,output=false;
  function capture(){
  ...
  function invalidate(){if(output)return;const p=document.querySelector('.m2-report-panel');if(p){p.dataset.rafexOutputVisible='0';delete p.dataset.rafexReadyV136;}window.__rafexFreeOutputDirty=true;}
  ...
  function draw(){
    ...
    let list=document.getElementById('rafexAreaList');if(!list){list=document.createElement('div');list.id='rafexAreaList';tools.insertAdjacentElement('afterend',list);}
  ...
  window.rafexRenderAreaOutput=async(target,render)=>{
    ...
    ensure();remember();const active=documentState.activeAreaId,areas=clone(documentState.areas),fragments=[],undo=m2UndoHistory;output=true;
    ...
    }finally{documentState.activeAreaId=active;restore(current().layout);m2UndoHistory=undo;m2UpdateUndoButton();output=false;page?.removeAttribute('inert');nav?.removeAttribute('inert');draw();}
  ...
  const style=document.createElement('style');style.textContent='#rafexAreaList{display:grid;gap:6px;flex-basis:100%;width:100%}.rafex-area-row{display:flex;gap:8px;padding:6px;border:1px solid #d5dfd9;border-radius:8px}.rafex-area-row.active{border-color:#214f3b;background:#eef5f0}.rafex-area-row input{flex:1;min-width:120px;font:inherit;padding:7px;border:1px solid #d5dfd9;border-radius:6px}.rafex-area-report-name{margin-left:auto;padding:4px 10px;color:#fff;max-width:45%;white-space:normal}';document.head.appendChild(style);
  ensure();draw();
})();
