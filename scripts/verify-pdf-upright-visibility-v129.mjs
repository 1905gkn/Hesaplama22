...
for (const required of [
  ...
  "paintLine(original,'rafex-pdf-upright-halo-v131','#002c45','5px')",
  "paintLine(original,'rafex-pdf-upright-overlay-v131','#0a8dcc','3px')",
...
scripts.forEach((entry, index) => {
  ...
  try { new vm.Script(entry[2], { filename:`v131-inline-${index}.js` }); }
  catch (error) { errors.push(String(error?.stack || error)); }
});
assert.equal(errors.length, 0, errors.join("\n"));
console.log("v131 verify: PDF ayakları kutulaşmadan 5 px koyu kontur ve 3 px mavi gövdeyle belirgin çiziliyor.");
