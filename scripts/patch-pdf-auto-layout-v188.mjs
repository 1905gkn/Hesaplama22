...
export function transform(html){
  ...
  const runtime=['imported-type-names.js','pdf-native-placement.js','imported-type-editor.js','pdf-auto-layout.js'].map(f=>fs.readFileSync(new URL('../client/'+f,import.meta.url),'utf8')).join('\n');
...
if(process.argv[1]?.replaceAll('\\','/').endsWith('/patch-pdf-auto-layout-v188.mjs')){
  ...
  for(const file of ['pdf-vector-reader.mjs','pdf-rack-detection.mjs','pdf-raster-reader.mjs','pdf-batch-plan.mjs','pdf-dimensioned-reader.mjs','pdf-selective-reader.mjs','pdf-analysis-review.mjs'])fs.copyFileSync('client/'+file,'dist/pdfjs/'+file);
