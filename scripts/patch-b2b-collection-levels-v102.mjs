...
<script data-rafex-b2b-collection-levels="v102">(function(){
  ...
  function patchViewer(viewer){
    ...
    viewer.addAccessories=function(section,sectionScale,depthScale){base.apply(this,arguments);var floors=Array.isArray(this.options.collectionFloors)?this.options.collectionFloors:[];if(!floors.length||!this.models?.mrTraverse||!this.models?.mrTray||typeof this.mrCollectionPart!=='function')return;var layer=new (this.content.constructor)();if(!layer||typeof layer.add!=='function')return;layer.name='B2B MR ZS Toplama Katlari';var clearLeft=126.70318603515625*sectionScale,clearWidth=this.options.sectionWidth,front=81.59595*depthScale,rear=1077.32687*depthScale,seat=Math.max(100,rear-front),self=this;
  ...
  window.addEventListener('click',function(event){var button=event.target?.closest?.('[data-b2b-collection-add]');if(!button)return;event.preventDefault();event.stopImmediatePropagation();window.rafexCollectionEnableV102()},true);
