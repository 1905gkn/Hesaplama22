...
<script data-rafex-b2b-collection-levels="v102">(function(){
  ...
  function freshFloor(){return{trayWidth:300,trayThickness:.8,traverse:"ZS55|1.5",height:500,load:200,selectionMode:"auto"}}
  ...
  function normalize(raw){
    ...
    return{enabled:raw.enabled===true,groundGap:Math.max(0,Math.min(5000,Number(raw.groundGap)||500)),floors:(list.length?list:[freshFloor()]).slice(0,12).map(function(f){var key=(/^ZS(35|55|65)\|(1\.5|2|2\.5)$/.test(f&&f.traverse||" ")||ZS[f&&f.traverse]||/^Kutu/.test(f&&f.traverse||""))?f.traverse:"",tw=[200,250,300].includes(Number(f&&f.trayWidth))?Number(f.trayWidth):300,th=[.6,.8,1,1.2,1.5].includes(Number(f&&f.trayThickness))?Number(f.trayThickness):.8;return{trayWidth:tw,trayThickness:th,traverse:key,load:Math.max(0,Number(f&&f.load)||0),selectionMode:f&&f.selectionMode||"manual",height:Math.max(100,Math.min(5000,Number(f&&f.height)||500))}})};
  }
  function selectionWidth(){return Number(window.b2bPalletGeometry?.().sectionWidth)||0;}
  function plan(){state.floors.forEach(function(f){window.RafexRackTravers?.updateFloor(f,selectionWidth())});var s=normalize(state),cursor=s.groundGap,rows=s.floors.map(function(f,index){var bottom=cursor,zsHeight=window.RafexRackTravers?.height(f.traverse)||ZS[f.traverse]||55;cursor+=zsHeight+f.height;return{index:index,bottom:bottom,zsHeight:zsHeight,top:cursor,trayWidth:f.trayWidth,trayThickness:f.trayThickness,traverse:f.traverse,height:f.height}});return{groundGap:s.groundGap,floors:rows,totalHeight:cursor}}
  ...
  function notify(){forceTraverse();try{window.b2bRefreshSummary?.()}catch(error){console.warn('Toplama kati ozeti',error)}try{var options=window.b2b3DOptions?.()||{};var p=plan();options.collectionFloors=clone(p.floors);options.firstPalletPosition='traverse';options.firstFloorGap=p.totalHeight;window.RafexB2BViewer?.update?.(options)}catch(error){console.warn('Toplama kati 3D',error)}queue()}
  ...
  function enableCollection(){if(!state.floors.length)state.floors=[freshFloor()];state.enabled=true;document.getElementById('b2bAccessoryArea')?.classList.add('open');render();notify()}window.rafexCollectionEnableV102=enableCollection;
  ...
  window.rafexCollectionFieldV102=function(index,key,value){var floor=state.floors[index];if(!floor)return;if(key==='traverse'){floor.traverse=value;floor.selectionMode='manual';}else if(key==='load'){floor.load=Math.max(0,Number(value)||0);floor.selectionMode='auto';}else if(key==='selectionMode')floor.selectionMode=value==='manual'?'manual':'auto';else if(key==='trayWidth')floor.trayWidth=[200,250,300].includes(Number(value))?Number(value):300;else if(key==='trayThickness')floor.trayThickness=[.6,.8,1,1.2,1.5].includes(Number(value))?Number(value):.8;else if(key==='height')floor.height=Math.max(100,Math.min(5000,Number(value)||500));render();notify()};
  document.addEventListener('change',function(event){var el=event.target;if(el?.dataset?.collectionField)window.rafexCollectionFieldV102(Number(el.dataset.collectionIndex),el.dataset.collectionField,el.value)});
  window.rafexCollectionStateV102=function(){return clone(normalize(state))};window.rafexCollectionPlanV102=plan;
  ['rafexCollectionEnableV102','rafexCollectionRemoveV102','rafexCollectionGroundV102','rafexCollectionSetCountV102','rafexCollectionCountV102','rafexCollectionFieldV102','rafexCollectionStateV102','rafexCollectionPlanV102'].forEach(function(name){var fn=window[name];try{Object.defineProperty(window,name,{value:fn,writable:false,configurable:false,enumerable:true})}catch(e){window[name]=fn}});
  ...
  function patchViewer(viewer){
    ...
    viewer.addAccessories=function(section,sectionScale,depthScale){base.apply(this,arguments);if(section.getObjectByName?.('B2B MR ZS Toplama Katlari'))return;var floors=Array.isArray(this.options.collectionFloors)?this.options.collectionFloors:[];if(!floors.length||!this.models?.mrTraverse||!this.models?.mrTray||typeof this.mrCollectionPart!=='function')return;var layer=new (this.content.constructor)();if(!layer||typeof layer.add!=='function')return;layer.name='B2B MR ZS Toplama Katlari';var clearLeft=126.70318603515625*sectionScale,clearWidth=this.options.sectionWidth,front=81.59595*depthScale,rear=1077.32687*depthScale,seat=Math.max(100,rear-front),self=this;
      floors.forEach(function(f){var height=Math.max(50,Number(f.zsHeight)||55),bottom=Math.max(0,Number(f.bottom)||0),beamDepth=Math.max(24,42*depthScale);[front,rear-beamDepth].forEach(function(offset,side){var beam=self.mrCollectionPart(self.models.mrTraverse,{x:clearWidth,y:beamDepth,z:height});beam.name=(f.traverse||'ZS35')+' MR Toplama '+(Number(f.index)+1)+' '+(side?'Arka':'On');beam.position.set(clearLeft,offset,-bottom);metalize(beam);layer.add(beam)});var cursor=0,pieces=typeof self.trayPiecePlan==='function'?self.trayPiecePlan(clearWidth,f.trayWidth):[];pieces.forEach(function(pieceWidth,pieceIndex){var tray=self.mrCollectionPart(self.models.mrTray,{x:pieceWidth,y:seat,z:35});tray.name='MR Toplama Tava '+(Number(f.index)+1)+'-'+(pieceIndex+1)+' · '+String(f.trayThickness).replace('.',',')+' mm';tray.position.set(clearLeft+cursor,front,-(bottom+height));metalize(tray);layer.add(tray);cursor+=pieceWidth});section.add(layer)});return viewer};
  }
...
if (!runtimeScript) throw new Error("B2B collection levels v102: runtime script bulunamadi");
