...
<script data-rafex-b2b-collection-levels="v102">(function(){
  ...
  function normalize(raw){
    ...
    return{enabled:raw.enabled===true,groundGap:Math.max(0,Math.min(5000,Number(raw.groundGap)||500)),floors:(list.length?list:[freshFloor()]).slice(0,12).map(function(f){var key=(ZS[f&&f.traverse]||/^Kutu/.test(f&&f.traverse||""))?f.traverse:"",tw=[200,250,300].includes(Number(f&&f.trayWidth))?Number(f.trayWidth):300,th=[.6,.8,1,1.2,1.5].includes(Number(f&&f.trayThickness))?Number(f.trayThickness):.8;return{trayWidth:tw,trayThickness:th,traverse:key,load:Math.max(0,Number(f&&f.load)||0),selectionMode:f&&f.selectionMode||"manual",height:Math.max(100,Math.min(5000,Number(f&&f.height)||500))}})};
  }
