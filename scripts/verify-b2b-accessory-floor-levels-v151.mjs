...
try{
  ...
  const check=async(position,expectedStop,expectedH)=>{
    await page.evaluate(value=>{
      ...
      rafexAccessoryRemove(1);rafexAccessoryRemove(0);
      rafexAccessoryAdd('palletStop');rafexAccessoryAdd('hTraverse');
    },position);
  ...
  const ground=await check('ground',['K1','K2','K3','K4','K5'],['K2','K3','K4','K5']);
  assert.deepEqual(ground,[{type:'palletStop',levels:[0,1,2,3,4]},{type:'hTraverse',levels:[1,2,3,4]}]);
  const traverse=await check('traverse',['K1','K2','K3','K4','K5'],['K1','K2','K3','K4','K5']);
  assert.deepEqual(traverse,[{type:'palletStop',levels:[1,2,3,4,5]},{type:'hTraverse',levels:[1,2,3,4,5]}]);
