...
import {transform} from './patch-static-deselection-v149.mjs';
const {chromium}=createRequire(import.meta.url)(process.env.PLAYWRIGHT_MODULE_PATH||'playwright');
const html=fs.readFileSync('.tmp-v109/build/dist/index.html','utf8');
const browser=await chromium.launch({channel:'msedge',headless:true});
try{for(const variant of html.includes('/* static-deselection-v149 */')?['optimized']:['current','optimized']){
  ...
  const deselections=[];
  for(const method of ['blank','escape','outside']){
    const target=page.locator('#m2LayoutSvg [data-rack="1040"] > .m2-layout-rack');
    await target.click();
    await page.evaluate(()=>{selectionStatsV148.renders=0;window.retainedRackV149=document.querySelector('#m2LayoutSvg [data-rack="1040"]')});
    const t=Date.now();
    if(method==='escape')await page.keyboard.press('Escape');
    else if(method==='outside')await page.locator('#m2FloorStatus').click();
    else {const box=await page.locator('#m2LayoutSvg').boundingBox();await page.mouse.click(box.x+box.width*.8,box.y+box.height*.85);}
    await page.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r))));
    const state=await page.evaluate(()=>({selected:m2LayoutState.selected,count:document.querySelectorAll('#m2LayoutSvg .m2-layout-rack.selected').length,renders:selectionStatsV148.renders,retained:retainedRackV149===document.querySelector('#m2LayoutSvg [data-rack="1040"]')}));
    assert.equal(state.selected,null);console.log(JSON.stringify({variant,method,...state}));
    if(variant==='optimized'){assert.equal(state.count,0);assert.equal(state.renders,0);assert(state.retained)}
    deselections.push({method,ms:Date.now()-t,...state});
  }
  console.log(JSON.stringify({variant,deselections}));  const result=await page.evaluate(()=>({...selectionStatsV148,positions:selectionStatsV148.positions===JSON.stringify(m2LayoutState.racks),history:selectionStatsV148.history===m2UndoHistory.length,drag:m2LayoutState.drag,selected:document.querySelectorAll('#m2LayoutSvg .m2-layout-rack.selected').length}));
  ...
  if(variant==='optimized'){assert.equal(result.motion,0);assert.equal(result.undo,0);assert.equal(result.renders,0);assert(result.positions);assert(result.history);assert.equal(result.drag,null);assert.equal(result.selected,0);}
  ...
  if(variant==='optimized'){
    await page.evaluate(()=>{m2PinnedDimensionsByRack['1040']={left:true};m2LayoutState.selected=1040;m2RenderLayout()});
    await page.keyboard.press('Escape');
    assert(await page.locator('#m2LayoutSvg .m2-wall-guide[data-wall-rack="1040"]').count(),'Pinned wall dimension remains after deselection');
    assert.equal(await page.locator('#m2LayoutSvg .m2-layout-rack.selected').count(),0);
    const fallback=await page.evaluate(()=>{const before=selectionStatsV148.renders;m2ClearAllSelections();return selectionStatsV148.renders-before});
    assert.equal(fallback,1,'Model-changing callers retain full render');
    console.log('Pinned dimension and model-change fallback passed');
  }
