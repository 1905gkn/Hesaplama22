...
const source=fs.readFileSync(process.argv[2]||'outputs/live-after-v167.html','utf8'),html=process.argv.includes('--live')?source:transform(source);
...
try{
 ...
 await page.locator('#m2LayoutZoomLabel').click();assert.deepEqual(await getView(),initial,'fit restores whole project');
