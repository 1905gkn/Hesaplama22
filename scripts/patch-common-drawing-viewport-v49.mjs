...
(function(){
  ...
  function decorate(){
    ...
    node.querySelectorAll("[data-rack]").forEach(function(group){
      ...
      var frameRect=frame&&frame.getBoundingClientRect(),shortPx=frameRect?Math.min(frameRect.width,frameRect.height):0,shortSide=rack?Math.max(1,Math.min(Number(rack.w)||1,Number(rack.h)||1)):12,fontSize=Math.max(8,Math.min(17,shortSide*.72)),selected=frame&&frame.classList.contains("selected"),labels=Array.from(group.querySelectorAll(".m2-b2b-plan-label,.m2-rack-name"));
      labels.forEach(function(label,index){label.style.fontSize=fontSize+"px";label.style.display=index===0?"":"none";label.style.opacity=".98";label.style.fill=color;label.style.stroke="#fff";label.style.strokeWidth=Math.max(1.1,fontSize*.12)+"px";label.style.paintOrder="stroke";label.style.fontWeight="950";label.style.letterSpacing=".04em";label.style.pointerEvents="none"});
