#!/usr/bin/env bash
# B2B safe level-based report build v40
set -euo pipefail
...
source = source.replace('const ASSET_VERSION = "b2b-detail-layout-camera-519";', 'const ASSET_VERSION = "b2b-safe-level-report-camera-584";');
...
let portalSource = fs.readFileSync(portalPath, 'utf8')
  ...
  .replaceAll('b2b-double-row-side-ties-367', 'b2b-safe-level-report-camera-584');
portalSource = portalSource.replace(/<\/body>\s*<\/html>\s*$/i, `<script data-rafex-b2b-visual-fixes="back-to-back-reference-v2">\n${b2bVisualFixes}\n</script>\n<script data-rafex-b2b-report-3d="front-side-capture-v35">\n${b2bReport3d}\n</script>\n<script data-rafex-b2b-report-sections="corporate-type-sections-v5">\n${b2bReportSections}\n</script>\n</body>\n</html>`);
if (!portalSource.includes('data-rafex-b2b-visual-fixes="back-to-back-reference-v2"') || !portalSource.includes('data-rafex-b2b-report-3d="front-side-capture-v35"') || !portalSource.includes('data-rafex-b2b-report-sections="corporate-type-sections-v5"')) {
