...
source = source.replace('const ASSET_VERSION = "b2b-detail-layout-camera-519";', 'const ASSET_VERSION = "b2b-contained-dimensions-533";');
...
let portalSource = fs.readFileSync(portalPath, 'utf8')
  ...
  .replaceAll('b2b-double-row-side-ties-367', 'b2b-contained-dimensions-533');
portalSource = portalSource.replace(/<\/body>\s*<\/html>\s*$/i, `<script data-rafex-b2b-visual-fixes="back-to-back-reference-v2">\n${b2bVisualFixes}\n</script>\n<script data-rafex-b2b-report-3d="front-side-capture-v21">\n${b2bReport3d}\n</script>\n</body>\n</html>`);
if (!portalSource.includes('data-rafex-b2b-visual-fixes="back-to-back-reference-v2"') || !portalSource.includes('data-rafex-b2b-report-3d="front-side-capture-v21"')) {
