...
if (!source.includes('const addSelectedLoad = (centerZ, supportTopY)')) {
  const loadHelpers = [
    ...
    "      else if (o.loadType === 'unpacked') return;",
...
if (!source.includes("dimensions.name = 'Konsol 3D Ölçüler';")) {
  const dimensions = [
    ...
    '      dimensions.add(line);',
    '    };',
    '    const addDimPoint = (position) => {',
    '      const marker = new THREE.Mesh(new THREE.SphereGeometry(24, 12, 8), new THREE.MeshBasicMaterial({ color: 0x2b6f91, depthTest: false }));',
    '      marker.position.copy(position);',
    '      marker.renderOrder = 102;',
    '      dimensions.add(marker);',
    '    };',
    ...
    "      context.fillStyle = 'rgba(5,40,72,.98)';",
    '      context.beginPath();',
    '      context.roundRect(0, 0, labelWidth, labelHeight, 18);',
    '      context.fill();',
    "      context.strokeStyle = '#3e8fb2';",
    '      context.lineWidth = 4;',
    '      context.stroke();',
    "      context.fillStyle = '#ffffff';",
    "      const markerText = String(text).startsWith('SON KAT') || String(text).startsWith('AYAK BOYU');",
    "      context.font = '900 ' + (markerText ? 72 : 84) + 'px Arial';",
    ...
    '      addDimPoint(bottom);',
    '      addDimPoint(top);',
    '      addDimLabel(label, x - 380, (from + to) / 2, dimensionZ, labelWidth);',
    '    };',
    '    const addLevelMarker = (x, height, label, labelWidth) => {',
    '      const point = new THREE.Vector3(witnessX, height, dimensionZ);',
    '      addDimLine([new THREE.Vector3(x, height, dimensionZ), point]);',
    '      addDimPoint(point);',
    '      addDimLabel(label, x - 420, height, dimensionZ, labelWidth);',
    '    };',
    ...
    "    addLevelMarker(totalX, visualUprightHeight, 'AYAK BOYU  ·  ' + Math.round(visualUprightHeight).toLocaleString('tr-TR') + ' mm', 880);",
