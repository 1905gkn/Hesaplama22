...
(()=>{
  ...
  const systemOf=(entry)=>{const d=entry?.drawing||entry||{},s=String(entry?.__rafexSystem||entry?.rafexSystem||d.rafexSystem||'').toLowerCase();if(['konsol','konsol-kollu','cantilever'].includes(s))return'konsol';if(s==='drive'||s==='drive-in'||s==='drivein')return'drive';if(s==='mr'||d?.b2b?.mr||d?.plan?.mr)return'mr';if(s==='b2b'||d.b2b||d.b2bLayout)return'b2b';return'mekik2'};
  ...
  const metaHtml=(entry,sys)=>{const d=entry.drawing||{};const label=sys==='b2b'?'B2B':sys==='mr'?'MR':sys==='drive'?'Drive-In':sys==='konsol'?'Konsol Kollu':'Mekik';const rows=[['Sistem',label],['Tip',entry.name||'Raf'],['Genişlik',(Math.round(Number(d.totalWidth||d.widthMm)||0)).toLocaleString('tr-TR')+' mm'],['Derinlik',(Math.round(Number(d.railLength||d.depthMm)||0)).toLocaleString('tr-TR')+' mm'],['Kat',Math.round(Number(d.levels)||0)],['Ayak',sys==='konsol'?Math.round(Number(d.konsol?.count||d.plan?.feet?.length)||0)+' adet':String(d.footProfile||d.b2b?.uprightType||d.footType||'-')]];return '<div class="rafex-free-info-meta">'+rows.map(([k,v])=>'<div><small>'+esc(k)+'</small><b>'+esc(v)+'</b></div>').join('')+'</div>'};
  const b2bOptions=(d)=>{let o={};try{o=typeof m2Rack3DOptions==='function'?(m2Rack3DOptions(d)||{}):{}}catch{}const b=d.b2b||{},l=d.b2bLayout||{};return{...o,moduleCount:1,moduleOptions:null,palletCount:Math.max(1,Math.min(4,Number(l.palletCount||b.palletCount||d.bays)||3)),palletWidth:Number(l.palletWidth||b.palletWidth||d.palW)||800,palletDepth:Number(l.palletDepth||b.palletDepth||d.palD)||1200,palletHeight:Number(d.palletHeight||b.palletHeight)||1200,levels:Math.max(1,Number(d.levels||b.levels)||4),rowType:(Number(l.rowCount)===2||b.rowType==='double')?'double':'single',rowGap:Number(l.rowGap??b.rowGap)||200,footHeight:Number(d.sideUprightHeight||d.totalRackHeight||b.footHeight)||undefined,showPallets:b.showPallets!==false,dimensions:{levels:true,markers:true,eye:true,width:true,depth:true}}};
  ...
  const konsolOptions=(d)=>{const k=d.konsol||d.spec||{},count=Math.max(2,Math.round(Number(k.count||d.plan?.feet?.length)||5));return{uprightCount:count,spacing:Number(k.spacing||d.palW)||1500,height:Number(k.height||d.totalRackHeight||d.sideUprightHeight)||4500,armLength:Number(k.arm||k.armLength||d.palD)||1200,baseDepth:Number(k.baseDepth||k.arm||d.palD)||1200,levels:Math.max(1,Math.round(Number(k.levels||d.levels)||4)),doubleSided:k.side==='double'||k.doubleSided===true,setPlan:Array.isArray(k.setPlan)?k.setPlan:[],armColor:k.armColor==='ral2004'?'ral2004':'ral1007',uprightProfile:k.uprightProfile||'ipe200',armProfile:k.armProfile||'npi100',productLength:Number(k.productLength)||3000,productHeight:Number(k.productHeight)||900,loadType:k.loadType||'profile'}};
  const viewButtons=(sys)=>'<div class="rafex-free-info-3d-actions"><button type="button" data-info-view="perspective" class="active">Perspektif</button><button type="button" data-info-view="front">Önden</button><button type="button" data-info-view="side">Yandan</button>'+(sys==='mr'||sys==='konsol'?'<button type="button" data-info-view="top">Üstten</button>':'')+'</div>';
  const systemLabel=(sys)=>sys==='b2b'?'B2B':sys==='mr'?'MR':'KONSOL KOLLU';
  const interactiveHtml=(entry,sys)=>metaHtml(entry,sys)+'<div class="rafex-free-info-3d"><div class="rafex-free-info-3d-head"><div><b>'+systemLabel(sys)+' · 3D MODÜL</b><small>Sol tuş: döndür · tekerlek: yakınlaştır · sağ tuş: kaydır · 3D ölçüler aktif</small></div>'+viewButtons(sys)+'</div><div class="rafex-free-info-canvas-wrap"><canvas class="rafex-free-info-canvas" aria-label="'+systemLabel(sys)+' interaktif 3D modül"></canvas><span class="rafex-free-info-3d-status">3D modül hazırlanıyor…</span></div></div>';
  ...
  async function mountInteractive(entry,sys,body,token){
    ...
    if(sys==='konsol'){
      if(typeof window.rafexLoadHeavyViewerV1==='function')await window.rafexLoadHeavyViewerV1('konsol');
      if(token!==activeInfoToken)return;
      const create=window.RafexKonsolViewer?.createDetached||window.RafexKonsolViewer?.mount;if(typeof create!=='function')throw new Error('Konsol interaktif 3D motoru hazır değil.');
      const viewer=create.call(window.RafexKonsolViewer,canvas,konsolOptions(entry.drawing));if(token!==activeInfoToken){viewer?.destroy?.();return}activeInfoViewer=viewer;bindViewerControls(body,viewer);status.textContent='Mouse ile döndür · tekerlek ile yakınlaştır · Konsol 3D görünümü';viewer.setView?.('perspective');return;
    }
...
for(const required of ['data-rafex-free-info-modules="v27"','3D MODÜL','createDetached','Konsol interaktif 3D motoru','3D ölçüler aktif','mekik-only','rafexFreeShowInfoV3=showInfo'])if(!html.includes(required))throw new Error('Bilgi modül v27 doğrulaması eksik: '+required);
const encoded=Buffer.from(html).toString('base64');
source=source.slice(0,match.index)+match[0].replace(match[2],encoded)+source.slice(match.index+match[0].length);
fs.writeFileSync(file,source);
console.log('Serbest bilgi penceresi: B2B/MR/Konsol mouse ile döndürülebilir 3D + 3D ölçüler; Mekik yalnız ön/yan (v28).');
