...
(()=>{
  ...
  function konsolInfo(entry){const d=entry.drawing||{},k=d.konsol||d.spec||{},count=Math.max(2,Math.round(Number(k.count)||5)),levels=Math.max(1,Math.round(Number(k.levels||d.levels)||4)),side=k.side==='double'?'Çift taraflı':'Tek taraflı',w=520,h=330,pad=42,usable=w-pad*2,base=h-35,top=28,step=usable/Math.max(1,count-1),levelStep=(base-top)/Math.max(1,levels);let svg=['<svg viewBox="0 0 '+w+' '+h+'" role="img" aria-label="Konsol Kollu teknik ön görünüş"><rect width="'+w+'" height="'+h+'" fill="#fff"/>'];for(let i=0;i<count;i++){let x=pad+i*step;svg.push('<rect x="'+(x-3)+'" y="'+top+'" width="6" height="'+(base-top)+'" fill="#005387"/>');for(let l=1;l<=levels;l++){let y=base-l*levelStep;svg.push('<rect x="'+x+'" y="'+(y-3)+'" width="'+Math.max(12,step*.72)+'" height="6" fill="#e1a100"/>')}}svg.push('<line x1="'+pad+'" y1="'+base+'" x2="'+(w-pad)+'" y2="'+base+'" stroke="#173c2d"/><text x="'+(w/2)+'" y="'+(h-8)+'" text-anchor="middle" font-size="12" font-weight="800" fill="#173c2d">'+esc(Math.round(Number(d.totalWidth)||0))+' × '+esc(Math.round(Number(d.railLength)||0))+' mm · '+esc(side)+'</text></svg>');return metaHtml(entry,'konsol')+'<div class="rafex-free-info-views"><div class="rafex-free-info-view"><strong>KONSOL KOLLU · ÖNDEN TEKNİK GÖRÜNÜŞ</strong>'+svg.join('')+'</div></div>'}
