(function(){
 ...
 function install(){
  ...
  function choose(){
   ...
   types.forEach((type,i)=>{const label=document.createElement('label'),check=document.createElement('input');check.type='checkbox';check.value=String(i);check.addEventListener('change',()=>copy.disabled=!list.querySelector('input:checked'));label.append(check,document.createTextNode((type.name||'Raf')+' · '+systemName(type)));list.append(label);});
