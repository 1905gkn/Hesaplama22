...
const undoPattern = /      function m2UndoSnapshot\(label="Son işlem"\)\{[\s\S]*?\n      function m2UndoLastAction\(\)\{[\s\S]*?\n      \}\n(?=      function )/;
const newUndo = String.raw`      let m2UndoBytes=0;const m2UndoMaxBytes=8*1024*1024,m2UndoMaxEntries=40;
      ...
      function m2PushPositionUndo(label="Raf taşıma"){
        ...
        try{(m2MultiSelect?.rackIds||[]).forEach((value)=>addId(rackIds,value));(m2MultiSelect?.symbolIds||[]).forEach((value)=>addId(symbolIds,value));}catch{}
...
if (!html.includes("m2UndoMaxBytes=8*1024*1024")) {
  if (!undoPattern.test(html)) throw new Error("Common source v90: undo blogu bulunamadi");
