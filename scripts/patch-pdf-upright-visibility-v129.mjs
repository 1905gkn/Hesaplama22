...
<style data-rafex-pdf-upright-visibility="v130">
...
:is(#m2ReportFloor,#m2A4PrintSheet,#m2A4PrintArea,#m2CorporatePreview,#m2CorporatePrint,#m2CorporatePrintArea) svg .rafex-pdf-upright-halo-v130{
  fill:#002c45!important;stroke:#002c45!important;stroke-width:14px!important;stroke-linejoin:miter!important;vector-effect:non-scaling-stroke!important;shape-rendering:crispEdges!important;opacity:1!important;filter:none!important
}
:is(#m2ReportFloor,#m2A4PrintSheet,#m2A4PrintArea,#m2CorporatePreview,#m2CorporatePrint,#m2CorporatePrintArea) svg .rafex-pdf-upright-overlay-v130{
  fill:#0a8dcc!important;stroke:#0a8dcc!important;stroke-width:10.5px!important;stroke-linejoin:miter!important;vector-effect:non-scaling-stroke!important;shape-rendering:crispEdges!important;opacity:1!important;filter:none!important
}
</style>
<script data-rafex-pdf-upright-visibility="v130">(function(){
  if(window.__rafexPdfUprightVisibilityV130)return;
  window.__rafexPdfUprightVisibilityV130=true;
  function paint(node,className,fill,strokeWidth){
    ...
    node.style.setProperty('fill',fill,'important');node.style.setProperty('fill-opacity','1','important');
    node.style.setProperty('stroke',fill,'important');node.style.setProperty('stroke-width',strokeWidth,'important');
    node.style.setProperty('stroke-opacity','1','important');node.style.setProperty('stroke-linejoin','miter','important');
  ...
  function enhance(svg){
    if(!svg)return 0;
    svg.querySelectorAll('.rafex-pdf-upright-overlay-v129,.rafex-pdf-upright-halo-v130,.rafex-pdf-upright-overlay-v130').forEach(function(node){node.remove()});
    ...
    originals.forEach(function(original){
      original.parentNode.appendChild(paint(original.cloneNode(true),'rafex-pdf-upright-halo-v130','#002c45','14px'));
      original.parentNode.appendChild(paint(original.cloneNode(true),'rafex-pdf-upright-overlay-v130','#0a8dcc','10.5px'));
    });
  ...
  if(baseA4){
    ...
    wrappedA4.__rafexPdfUprightVisibilityV130=true;m2RenderA4Report=wrappedA4;window.m2RenderA4Report=wrappedA4;
  }
  ...
  if(baseBuild){
    ...
    wrappedBuild.__rafexPdfUprightVisibilityV130=true;m2BuildCorporatePages=wrappedBuild;window.m2BuildCorporatePages=wrappedBuild;
  }
  window.rafexPdfUprightVisibilityV130={enhance:enhance,enhanceHost:enhanceHost};
...
for (const required of [
  ...
  "rafexPdfUprightVisibilityV130",
]) if (!html.includes(required)) throw new Error("PDF upright visibility v130 missing: " + required);
...
console.log("v130: PDF ust gorunum ayaklari iki katmanli, keskin ve baskin isaretlerle belirginlestirildi.");
