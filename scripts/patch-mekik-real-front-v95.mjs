...
async function buildExactGlb() {
  ...
  for (const name of ["part-00", "part-01", "part-02"]) {
    ...
    const gzipPart = Buffer.from(encoded, "base64");
    if (gzipPart[0] !== 0x1f || gzipPart[1] !== 0x8b) throw new Error(`v95: ${name} gzip degil`);
    const rawPart = zlib.gunzipSync(gzipPart);
    decodedParts.push(rawPart);
    console.log(`v95: ${name} cozuldu: ${rawPart.length} bytes`);
  }
  const glb = Buffer.concat(decodedParts);
  if (glb.subarray(0, 4).toString("ascii") !== "glTF") throw new Error(`v95: exact GLB glTF imzasi gecersiz: ${glb.subarray(0,4).toString("hex")}`);
  const declaredLength = glb.readUInt32LE(8);
  if (declaredLength !== glb.length) throw new Error(`v95: exact GLB uzunluk uyusmuyor declared=${declaredLength} actual=${glb.length}`);
...
const boot = '<meta data-rafex-mekik-real-glb-front="v96-exact"><script defer src="/mekik-front-viewer.js?v=v96-exact"></script>';
...
for (const required of ['MEKIK_FRONT_VIEWER_V95_BASE64','MEKIKSON2_EXACT_GLB_BASE64','path === "/mekikson2.glb"']) {
...
console.log("v96-exact: Mekik ön görünüş exact GLB route ile hazır; B2B/üst/yan görünüş değiştirilmedi.");
