...
for (const required of ["rafexCommonSingleLineLetterV58","rafex-single-line-letter-v58","fill:none;stroke-width:1.5px","baseSize=11","vector-effect:none",'path.setAttribute("d",pathData)','path.setAttribute("stroke",strokeColor)','label.style.display="none"',"contrastColor"]) if (!html.includes(required)) throw new Error("Serbest Çizim kalın tek-hat blok harfi bulunamadı: " + required);
...
for (const required of ["Tip yazı oranı","Renk kontrastı","rafexCommonTypeLetterScaleV65","rafexCommonTypeLetterContrastV65","rafexSetTypeLetterScaleV65","rafexSetTypeLetterContrastV65","rafex-type-letter-scale-v65",'min="10" max="200" step="5"','min="50" max="150" step="5"','Math.max(10,Math.min(200']) if (!html.includes(required)) throw new Error("Serbest Çizim tip yazı ve renk kontrastı özelliği bulunamadı: " + required);
for (const required of ['Math.max(.1,Math.min(2','rafex-single-line-system-v58','return "Drive-In"','return "Mekik"','systemFont=4.2*scale']) if (!html.includes(required)) throw new Error("Serbest Çizim Drive-In/Mekik ölçekli sistem etiketi bulunamadı: " + required);
...
for (const required of ["target.depth=geometry.rowCount", "target.depthMm=geometry.footprintDepth", "target.railLength=geometry.footprintDepth", "target.h=Math.max(.5,geometry.footprintDepth", "state.rowType==='double'?2:0"]) if (!html.includes(required)) throw new Error("MR uzatma sıra geometrisi bulunamadı: " + required);
...
for (const required of ["['b2b','mekik2','drive','mr','konsol']","else if(target==='konsol')renderKonsolCommon()","rafexSystem:'konsol'","{key:'konsol',label:'Konsol Kollu'}","/api/rack-types","layoutView='konsol-top'"]) if (!html.includes(required)) throw new Error(`Konsol birleşik Ortak Çizim entegrasyonu bulunamadı: ${required}`);
