...
if (!html.includes('/drive-in-viewer.js?v=drive-in-front-v10')) throw new Error("Drive In viewer lazy yükleyicisi canlı HTML içinde bulunamadı");
...
for (const required of ["['b2b','mekik2','drive','mr','konsol']","else if(target==='konsol')renderKonsolCommon()","rafexSystem:'konsol'","{key:'konsol',label:'Konsol Kollu',api:'/api/mekik2-types'}","layoutView='konsol-top'"]) if (!html.includes(required)) throw new Error(`Konsol birleşik Ortak Çizim entegrasyonu bulunamadı: ${required}`);
