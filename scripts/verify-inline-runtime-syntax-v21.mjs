...
for (const required of ['label.style.display=index===0?"":"none"','label.style.fill=color','label.style.paintOrder="stroke"']) if (!html.includes(required)) throw new Error("Serbest Çizim blok harfi görünürlüğü bulunamadı: " + required);
for (const required of ["Math.max(7,Math.min(14,shortSide*.64))",'label.style.opacity=".82"','label.style.fontWeight="800"','label.style.letterSpacing=".02em"']) if (!html.includes(required)) throw new Error("Serbest Çizim dengeli blok harfi görünümü bulunamadı: " + required);
