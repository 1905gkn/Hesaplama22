...
(()=>{
  ...
  let genericGapPairV26=null;
  const rememberGenericGapPair=()=>{try{const owner=m2MeasurementRack(),nearest=owner&&m2NearestRackGap(owner);if(owner&&nearest?.other)genericGapPairV26={ownerId:Number(owner.id),otherId:Number(nearest.other.id)}}catch(error){}};
  document.addEventListener('focusin',(event)=>{const input=event.target?.closest?.('.m2-wall-editor input[type="number"]');if(input&&String(input.getAttribute('aria-label')||'').toLocaleLowerCase('tr-TR').includes('en yakın raf arası'))rememberGenericGapPair()},true);
  try{const previousGenericGap=window.m2SetRackCenterDistance;const stableGenericGap=function(rawValue){let owner=(m2LayoutState?.racks||[]).find((rack)=>Number(rack.id)===Number(genericGapPairV26?.ownerId)),other=(m2LayoutState?.racks||[]).find((rack)=>Number(rack.id)===Number(genericGapPairV26?.otherId));if(!owner||!other){rememberGenericGapPair();owner=(m2LayoutState?.racks||[]).find((rack)=>Number(rack.id)===Number(genericGapPairV26?.ownerId));other=(m2LayoutState?.racks||[]).find((rack)=>Number(rack.id)===Number(genericGapPairV26?.otherId))}const relation=owner&&other?directRackGap(owner,other):null;if(!relation){if(typeof previousGenericGap==='function')return previousGenericGap.apply(this,arguments);return}const displayMm=Math.max(0,Number(rawValue)||0),desiredPx=(displayMm+(Number(relation.clearanceMm)||0))*Math.max(.0001,Number(m2LayoutState.scale)||.04),delta=desiredPx-Number(relation.distance||0);let nextX=owner.x,nextY=owner.y;if(relation.direction==='right')nextX-=delta;else if(relation.direction==='left')nextX+=delta;else if(relation.direction==='bottom')nextY-=delta;else if(relation.direction==='top')nextY+=delta;const moved=m2MoveRackOrJoinedGroup(owner,nextX,nextY),status=document.getElementById('m2FloorStatus');if(moved){try{m2RememberEditedDistance(owner,'gap')}catch(error){}if(status)status.textContent='Raf arası '+formatMm(displayMm)+' mm olarak ayarlandı.'}else if(status)status.textContent='Girilen raf arası mesafe bloğu alan dışına çıkarıyor veya başka bir rafla çakıştırıyor.';m2RenderLayout()};stableGenericGap.__rafexStablePairV26=true;try{m2SetRackCenterDistance=stableGenericGap}catch{}window.m2SetRackCenterDistance=stableGenericGap}catch(error){console.warn('Serbest çizim raf arası sabitlenemedi',error)}
...
for (const required of ['data-rafex-reference-front="v26"','__rafexMekikFootSplitV26','directRackGap','genericGapPairV26','Ayak takımı','Ayak profili']) {
