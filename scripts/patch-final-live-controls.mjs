...
// 1) Arasi Olc: source'ta bulunan kesin calisan buton/runtime'i final production HTML'ine de zorla tasi.
...
// 2) Aksesuar katlari: eski pointerdown yolunu tamamen temizle. Tek tek kat secimi sadece tek click yolundan calissin.
html=html.replace(/\n\s*document\.addEventListener\('pointerdown',function\(event\)\{[\s\S]*?m2ToggleCustomizeRackAccessoryLevel\(type,level\);\s*\},true\);/g,'\n');

// Mevcut capture click handler kat butonunu yakalamasin; butonun kendi onclick'i tek kez calissin.
...
// 3) Zemin rafinda K1 yalniz H Travers ve Tava icin yok. Palet Dayama K1 korunur.
...
if(!html.includes('data-level=\\"\'+level+\'\\"')&&!html.includes('data-level=\"\'+level+\'\"'))throw new Error("FINAL: Kat butonlarina tekil data-level eklenemedi.");
