...
const dims={sectionWidth:2700,frameDepth:1100,footHeight:9500,levels:5,palletCount:3,palletWidth:800,palletDepth:1200,palletHeight:1900,palletWeight:800,firstBeamTop:2200,clearOpening:2050,doubleRowGap:300};
...
if(process.env.RAFEX_RASTER_FIXTURE){
  ...
  const g=detectRasterGeometry(ctx.getImageData(0,0,image.width,image.height));assert.equal(g.rows.length,10);assert(g.count>400&&g.count<500);const grouped=groupRackPlan(rasterPlan(g,dims));assert.equal(grouped.importTypes.length,2);assert.equal(grouped.importTypes.filter(t=>t.rowType==='double').length,1);assert.equal(grouped.importTypes.find(t=>t.rowType==='double').rowGap,300);console.log('PASS same raster racks share one double-row type despite pixel gap differences.');console.log('PASS raster fixture: 10 rows, '+g.count+' candidates; no filename or image-coordinate rules.');
