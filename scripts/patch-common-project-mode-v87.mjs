...
<script data-rafex-common-project-mode="v87">(()=>{
  ...
  let commonName='',commonLocked=false,explicitNavExit=false,refreshing=false;
  ...
  function ensureCommonNameField(){
    ...
    if(!input){
      ...
      input?.addEventListener('input',()=>{commonName=input.value;});
    }
    if(input&&input.value!==commonName)input.value=commonName;
  }
  function lockCommonProject(project,asCopy=false){
    commonLocked=true;
    const name=String(project?.project_name||'')+(asCopy?' - Kopya':'');setCommonName(name);
  ...
  document.addEventListener('click',(event)=>{
    ...
    if(button.dataset.page==='free'){commonLocked=true;requestAnimationFrame(ensureCommonNameField);return;}
    explicitNavExit=true;commonLocked=false;setTimeout(()=>{explicitNavExit=false;},0);
  },true);
  const previousShowPage=window.showPage;
  if(typeof previousShowPage==='function'&&!previousShowPage.__rafexCommonProjectV87){
    const wrapped=function(name){
      if(commonLocked&&!explicitNavExit&&['b2b','mr','mekik2'].includes(String(name)))return;
      if(String(name)==='free')commonLocked=true;
  ...
  if(typeof previousLoadProject==='function'&&!previousLoadProject.__rafexCommonProjectV87){
    const wrapped=function(id){
      const project=(Array.isArray(window.m2ProjectRecords)?window.m2ProjectRecords:typeof m2ProjectRecords!=='undefined'?m2ProjectRecords:[]).find((item)=>Number(item?.id)===Number(id));
  ...
  if(typeof previousRefreshProjects==='function'&&!previousRefreshProjects.__rafexCommonProjectV87){
    const wrapped=async function(){
      ...
      refreshing=true;
      try{
        const result=await window.req('/api/projects');
        const rows=(result?.projects||[]).filter(isCommonProject);
        try{m2ProjectRecords=rows}catch{}window.m2ProjectRecords=rows;
        window.m2RenderProjects?.();try{m2RenderProjects?.()}catch{}
      }catch(error){const status=document.getElementById('m2FloorStatus');if(status)status.textContent=error?.message||'Ortak projeler getirilemedi.';}
  ...
  if(typeof previousSaveProject==='function'&&!previousSaveProject.__rafexCommonProjectV87){
    const wrapped=function(){
      if(commonActive()){
        commonLocked=true;ensureCommonNameField();const name=String(commonInput()?.value||commonName||'').trim();
  ...
  const observer=new MutationObserver(()=>{if(pageIsCommon())ensureCommonNameField();});observer.observe(document.documentElement,{childList:true,subtree:true});
  if(pageIsCommon()){commonLocked=true;ensureCommonNameField();}
  window.rafexIsCommonProjectV87=isCommonProject;
  window.rafexOpenCommonProjectV87=openCommonProject;
})();</script>`;
...
console.log('v87: Ortak Cizim modu kilitlendi; proje adi ayrildi; kayit module=ortak olarak zorlandi.');
