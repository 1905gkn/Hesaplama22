...
patchFile("client/b2b-viewer.entry.js", (src) => {
  ...
  const needle = "window.RafexB2BViewer = {";
  if (!src.includes(needle)) throw new Error("v30: B2B detached API kok noktasi bulunamadi.");
  const method = `window.RafexB2BViewer = {\n  createDetached(canvas, options) {\n    if (!(canvas instanceof HTMLCanvasElement)) throw new Error(\"B2B bilgi 3D tuvali bulunamadı.\");\n    return new B2BViewer(canvas, options || {});\n  },`;
  src = src.replace(needle, method);
  if (!src.includes("createDetached(canvas, options)")) throw new Error("v30: B2B detached API eklenemedi.");
  return src;
});

patchFile("client/mr-viewer.entry.js", (src) => {
  if (src.includes("createDetached(canvas, config)")) return src;
  const needle = "window.RafexMRViewer = {";
  if (!src.includes(needle)) throw new Error("v30: MR detached API kok noktasi bulunamadi.");
  const method = `window.RafexMRViewer = {\n  createDetached(canvas, config) {\n    if (!(canvas instanceof HTMLCanvasElement)) throw new Error(\"MR bilgi 3D tuvali bulunamadı.\");\n    return new MRViewer(canvas, { config: config || {} });\n  },`;
  src = src.replace(needle, method);
  if (!src.includes("createDetached(canvas, config)")) throw new Error("v30: MR detached API eklenemedi.");
  return src;
});
