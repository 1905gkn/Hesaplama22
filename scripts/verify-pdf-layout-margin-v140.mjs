...
import {transform} from './patch-pdf-layout-margin-v140.mjs';
const source=fs.readFileSync(process.argv[2]||'.perf-production.html','utf8'),m=source.match(/const\s+HTML_BASE64\s*=\s*(["'])([A-Za-z0-9+/=]+)\1/);
const html=transform(d(c(b(a(m?Buffer.from(m[2],'base64').toString():source)))));
...
try{
 ...
 for(const [w,h] of [[100000,100000],[200000,200000],[200000,100000]]){
  ...
  await page.waitForFunction(()=>document.querySelector('#m2ReportFloor svg[data-rafex-pdf-margin="v140"]'),{},{timeout:5000});
  for(const zoom of [0,-1,1]){
   ...
   const [x,y,vw,vh]=result.v,[l,t,r,b]=result.b;for(const margin of [(l-x)/vw,(t-y)/vh,(x+vw-r)/vw,(y+vh-b)/vh])assert(margin>=.1-1e-9);
 ...
 await page.evaluate(async()=>{window.print=()=>{};window.__rafexPrepareCorporatePrint=async()=>{};await m2PrintCorporateReport();});await page.waitForTimeout(800);await page.emulateMedia({media:'print'});
 ...
 assert.equal(await page.locator('#m2A4PrintSheet svg[data-rafex-pdf-margin="v140"]').count(),1,'summary print uses the same fitted export');
 const summaryMargins=await page.evaluate(()=>{const svg=document.querySelector('#m2A4PrintSheet svg[data-rafex-pdf-margin="v140"]'),p=document.getElementById('m2A4PrintSheet').getBoundingClientRect(),r=svg.getBoundingClientRect(),v=svg.getAttribute('viewBox').split(' ').map(Number),b=svg.getAttribute('data-rafex-pdf-bounds').split(' ').map(Number),s=Math.min(r.width/v[2],r.height/v[3]),ox=r.left+(r.width-v[2]*s)/2,oy=r.top+(r.height-v[3]*s)/2;return[(ox+(b[0]-v[0])*s-p.left)/p.width,(p.right-ox-(b[2]-v[0])*s)/p.width,(oy+(b[1]-v[1])*s-p.top)/p.height,(p.bottom-oy-(b[3]-v[1])*s)/p.height];});
 ...
 assert.deepEqual(errors,[]);assert.deepEqual(writes,[]);console.log('PASS PDF v140');
