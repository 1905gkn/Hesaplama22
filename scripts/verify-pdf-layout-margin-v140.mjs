...
import {transform as tightFit} from './patch-pdf-tight-fit-v141.mjs';
import {transform} from './patch-pdf-page-ruler-v142.mjs';
const source=fs.readFileSync(process.argv[2]||'.perf-production.html','utf8'),m=source.match(/const\s+HTML_BASE64\s*=\s*(["'])([A-Za-z0-9+/=]+)\1/);
const html=transform(tightFit(margins(d(c(b(a(m?Buffer.from(m[2],'base64').toString():source)))))));
...
try{
 ...
 async function checkRuler(selector){
  await page.waitForTimeout(100);
  const result=await page.locator(selector).evaluate(svg=>{
   const sheet=svg.closest('.m2-corporate-page,.m2-a4-sheet'),page=sheet.getBoundingClientRect(),ruler=sheet.querySelector('[data-rafex-page-ruler] .m2-metre-ruler'),r=ruler.getBoundingClientRect(),a=svg.getScreenCTM(),b=ruler.getScreenCTM();
   return{left:(r.left-page.left)/page.width,bottom:(page.bottom-r.bottom)/page.height,scale:b.a/a.a,verticalScale:b.d/a.d};
  });
  console.log('RULER',selector,result);assert(Math.abs(result.left-.03)<.002);assert(Math.abs(result.bottom-.03)<.002);assert(Math.abs(result.scale-1)<1e-6);assert(Math.abs(result.verticalScale-1)<1e-6);
 }
 ...
 for(const [w,h] of [[200000,100000],[100000,100000],[200000,200000]]){
  ...
  await checkRuler('#m2ReportFloor svg');
  await page.setViewportSize({width:1100,height:850});await checkRuler('#m2ReportFloor svg');await page.setViewportSize({width:1440,height:1000});
 ...
 await checkRuler('#m2CorporatePrint .m2-corporate-floor svg');
 ...
 await checkRuler('#m2A4PrintSheet svg[data-rafex-pdf-margin="v141"]');
 await page.pdf({path:'tmp/pdfs/layout-margin-v140-summary.pdf',printBackground:true,preferCSSPageSize:true});
 assert.deepEqual(errors,[]);assert.deepEqual(writes,[]);console.log('PASS PDF v142: page-fixed metric ruler, centered maximal fit, no editor mutations');
