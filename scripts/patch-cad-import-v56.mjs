...
<style ${marker}>
  ...
  .rafex-cad-edge-actions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;padding:0 20px 18px}.rafex-cad-edge-actions button{padding:10px 8px}.rafex-cad-edge-actions .danger{background:#fff0f0;color:#a12626;border:1px solid #e7b7b7}
...
(function(){
  ...
  function ensureEdgeDialog(){var dialog=document.getElementById('rafexCadEdgeDialog');if(dialog)return dialog;document.body.insertAdjacentHTML('beforeend','<dialog class="rafex-cad-edge-dialog" id="rafexCadEdgeDialog"><div class="rafex-cad-edge-head"><b>Kenar Uzunluk Bilgisi</b><small>Seçilen kenarın uzunluğunu değiştirebilir veya kenarı silebilirsin.</small></div><div class="rafex-cad-edge-body"><label>Kenar uzunluğu (mm)<input id="rafexCadEdgeLength" type="number" min="1" step="1"></label></div><div class="rafex-cad-edge-actions"><button type="button" onclick="rafexCloseCadEdgeV57()">Vazgeç</button><button type="button" class="danger" onclick="rafexDeleteCadEdgeV57()">Kenarı Sil</button><button type="button" onclick="rafexApplyCadEdgeV57()">Ölçüyü Uygula</button></div></dialog>');return document.getElementById('rafexCadEdgeDialog');}
  function openEdgeDialog(index){var points=m2LayoutState.points||[],start=points[index],end=points[(index+1)%points.length];if(!start||!end)return;var dialog=ensureEdgeDialog(),input=document.getElementById('rafexCadEdgeLength');dialog.dataset.edge=String(index);input.value=String(Math.max(1,Math.round(Math.hypot(end.x-start.x,end.y-start.y)/Math.max(.0001,Number(m2LayoutState.scale)||.04))));dialog.showModal();setTimeout(function(){input.focus();input.select();},20);}
  ...
  document.addEventListener('dblclick',function(event){var edge=event.target?.closest?.('[data-cad-edge]');if(!edge)return;event.preventDefault();event.stopImmediatePropagation();openEdgeDialog(Number(edge.dataset.cadEdge));},true);
...
for (const required of [marker, "AutoCAD’den Aktar", "rafex_alan:'area'", "pathBreaks.push(layoutPoints.length-1)", "Kenar Uzunluk Bilgisi", "rafexDeleteCadEdgeV57", "payload?.projectName", "window.rafexImportCadFileV56"]) {
