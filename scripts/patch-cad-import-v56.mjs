...
(function(){
  ...
  async function importFile(file){
    if(!file)return;try{
      ...
      var lineLength=function(points){var total=0;for(var i=1;i<points.length;i++)total+=Math.hypot(points[i].x-points[i-1].x,points[i].y-points[i-1].y);return total;},areas=objects.filter(function(object){return object.type==='area'&&object.points.length>=2;});if(!areas.length)throw new Error('RAFEX_ALAN sınırı bulunamadı. CAD’de önce Alan Sınırı ile açık veya kapalı bir çizgi oluştur.');areas.sort(function(a,b){var scoreA=a.closed&&a.points.length>=3?polygonArea(a.points):lineLength(a.points),scoreB=b.closed&&b.points.length>=3?polygonArea(b.points):lineLength(b.points);return scoreB-scoreA;});var area=areas[0],areaPoints=area.points.slice(),first=areaPoints[0],last=areaPoints[areaPoints.length-1];if(area.closed&&areaPoints.length>3&&Math.hypot(first.x-last.x,first.y-last.y)<.001)areaPoints.pop();
...
for (const required of [marker, "AutoCAD’den Aktar", "rafex_alan:'area'", "!area.closed||touchesArea(object,areaPoints)", "payload?.projectName", "window.rafexImportCadFileV56"]) {
