...
function syncMainHeightMode(){
 const input=document.getElementById('b2bPalletHeight');if(!input||!document.createElement)return;
 const active=mainRows.length>0;if(input.readOnly!==active)input.readOnly=active;
 const id='rafexManualHeightNoticeV163';let note=document.getElementById(id);
 if(active&&!note){note=document.createElement('button');note.id=id;note.type='button';note.style.cssText='grid-column:1/-1;width:100%;font-size:11px;padding:8px;margin-top:6px;white-space:normal';note.onclick=()=>open('main');input.closest('label').after(note);}
 if(note){if(note.hidden===active)note.hidden=!active;const text='Manuel kat yükseklikleri: '+mainRows.map(r=>r.palletHeight+' mm').join(' / ')+'. Düzenlemek veya otomatik düzene dönmek için tıkla.';if(active&&note.textContent!==text)note.textContent=text;}
 if(active){if(input.getAttribute('aria-describedby')!==id)input.setAttribute('aria-describedby',id);const title='Manuel kat düzeni etkin. Kat yüksekliklerini Manuel yükseklik bölümünden değiştirin.';if(input.title!==title)input.title=title;}else{if(input.hasAttribute('aria-describedby'))input.removeAttribute('aria-describedby');if(input.hasAttribute('title'))input.removeAttribute('title');}
}
...
function open(kind){
 ...
 const displayed=saved.map((r,i)=>({...r,traverseType:ground?saved[i-1]?.traverseType:r.traverseType,selectionMode:ground?saved[i-1]?.selectionMode:r.selectionMode}));
 ...
 dialog.innerHTML='<h3>Manuel yükseklik</h3><p>Her satırdaki yük, palet yüksekliği ve travers seçimi aynı kata aittir. Zemin paleti yerdeyse ilk satırda travers seçilmez; Travers üstü seçildiğinde zemin katı travers seçimi açılır. Kat arası mesafe, alt traversin üstünden üst traversin altına kadar olan net boşluktur; travers yüksekliği dahil değildir. Zemin mesafesi ilk traversin altına kadardır. Kat ağırlığı bir sıradaki katın toplam yüküdür.</p><div class="rows">'+Array.from({length:count},(_,i)=>{const r=saved[i]||{},hasDistance=!ground||i<count-1,distance=r.distance??(i===0?physical[0]?.bottom??200:(physical[i]?.bottom-physical[i-1]?.bottom-physical[i-1]?.beam)||pallet+200),levelName=ground?(i===0?'Zemin katı':i+'. kat'):(i+1)+'. kat',distanceName=i===0?'Zemin – 1. kat':i+'. kat – '+(i+1)+'. kat',traverseName=ground?(i===0?'Zemin katı — travers yok':i+'. kat travers tipi'):(i===0?'Zemin katı travers tipi':levelName+' travers tipi'),hasTraverse=!ground||i>0;return '<div class="level-row"><label>'+(hasDistance?distanceName+' mesafesi (mm)':'Üst kat mesafesi')+(hasDistance?'<input data-distance type="number" min="0" max="30000" value="'+distance+'">':'<input data-distance type="number" value="0" disabled>')+'</label><label>'+levelName+' ağırlığı (kg)<input data-weight type="number" min="0" max="100000" value="'+(r.weight??weight)+'"></label><label>'+levelName+' palet yüksekliği (mm)<input data-pallet type="number" min="300" max="3000" value="'+(r.palletHeight||pallet)+'"></label><label>'+traverseName+'<select data-traverse'+(hasTraverse?'':' disabled')+'>'+['CC100','CC120','CC140','CC160','CC180'].map(t=>'<option'+(t===(displayed[i]?.traverseType||rack?.b2b?.traverseType||'CC140')?' selected':'')+'>'+t+'</option>').join('')+'</select></label></div>'}).join('')+'</div><p role="alert" class="error"></p><footer><button data-clear>Otomatik düzene dön</button><button data-cancel>Vazgeç</button><button data-save>Uygula</button></footer>';
 dialog.querySelector('[data-cancel]').onclick=()=>dialog.close();
 const commit=rows=>{if(custom){customRows=rows;customCleared=!rows.length;const legacy=document.getElementById('m2CustomizeManualLevels');if(legacy)legacy.checked=rows.length>0}else{mainRows=rows;syncMainHeightMode();}dialog.close();if(custom)window.rafexUpdateB2BCustomizeViewerV119?.();else b2bApplyInputs();};
...
const restore=b2bApplySavedInputState;b2bApplySavedInputState=window.b2bApplySavedInputState=function(s){mainRows=copy(s?.manualLevelSpecs||[]);const result=restore.apply(this,arguments);syncMainHeightMode();return result};
const options=b2b3DOptions;b2b3DOptions=window.b2b3DOptions=function(){const base=options.apply(this,arguments);syncMainHeightMode();return mainRows.length?manualOptions(base,mainRows):{...automaticOptions(base),firstFloorGap:base.firstFloorGap}};
...
const foot=b2bFootCalculationInputs;b2bFootCalculationInputs=window.b2bFootCalculationInputs=function(){const old=foot.apply(this,arguments),collection=window.rafexCollectionFootLoads?.();if(!mainRows.length&&!collection?.enabled)return old;const position=collection?.enabled?'traverse':document.getElementById('b2bFirstPalletPosition')?.value==='traverse'?'traverse':'ground',rows=mainRows.slice(0,value('b2bLevels',1)),loaded=position==='ground'?rows.slice(1):rows,loadedLevels=rows.length?loaded.length:value('b2bLevels',1),loadedPallets=loadedLevels*value('b2bPalletCount',3),total=rows.length?loaded.reduce((n,r)=>n+r.weight,0):loadedPallets*value('b2bPalletWeight',0),collectionLoad=collection?.enabled?collection.load:0;return {...old,position,loadedLevels,loadedPallets,palletOnlyLoad:total,totalPalletLoad:total+collectionLoad,collectionLoad,footLoad:total+collectionLoad,footCalculationHeight:collection?.enabled?collection.firstSupportHeight:rows[0].distance+(Number(String(rows[0].traverseType).match(/\d+/)?.[0])||140)}};
})();</script>`;
