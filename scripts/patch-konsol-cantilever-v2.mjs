...
(function(){
  ...
  function currentState(){var plan=setPlan(num('konsolUprightCount',5));return{uprightCount:Math.max(2,num('konsolUprightCount',5)),spacing:Math.max(300,num('konsolSpacing',1500)),height:Math.max(1000,num('konsolHeight',4000)),armLength:Math.max(250,num('konsolArmLength',1000)),productLength:Math.max(500,num('femProductLength',3000)),productHeight:100,liftClearance:Math.max(0,num('konsolLiftClearance',100)),baseDepth:Math.max(250,num('konsolBaseDepth',1000)),armColor:el('konsolArmColor')&&el('konsolArmColor').value==='ral2004'?'ral2004':'ral1007',uprightProfile:el('konsolUprightProfile')&&el('konsolUprightProfile').value||'ipe200',armProfile:el('konsolArmProfile')&&el('konsolArmProfile').value||'npi100',levels:Math.max(1,num('konsolLevels',4)),doubleSided:sideKey()==='double',setPlan:plan,angle:num('konsolAngle',35)}}
