...
const loader='<script data-rafex-konsol-viewer-loader="v2" defer src="/konsol-viewer.js?v=konsol-krs-v12"></script>';
...
(function(){
  ...
  function setPlan(count){var n=Math.max(2,Math.round(Number(count)||2)),table={2:[2],3:[3],4:[4],5:[5],6:[4,2],7:[4,3],8:[4,4],9:[4,5],10:[5,5],11:[4,4,3],12:[4,4,4]},out=[];while(n>12){out.push(4);n-=4}return out.concat(table[n]||[n])}
  ...
  function currentState(){var plan=setPlan(num('konsolUprightCount',5)),lift=Math.max(0,num('konsolLiftClearance',100)),gap=Math.max(100,num('konsolLevelGap',1000));return{uprightCount:Math.max(2,num('konsolUprightCount',5)),spacing:Math.max(300,num('konsolSpacing',1500)),height:Math.max(1000,num('konsolHeight',4000)),armLength:Math.max(250,num('konsolArmLength',1000)),productLength:Math.max(500,num('femProductLength',3000)),productHeight:Math.max(50,gap-lift),liftClearance:lift,baseDepth:Math.max(250,num('konsolBaseDepth',1000)),armColor:el('konsolArmColor')&&el('konsolArmColor').value==='ral2004'?'ral2004':'ral1007',uprightProfile:el('konsolUprightProfile')&&el('konsolUprightProfile').value||'ipe200',armProfile:el('konsolArmProfile')&&el('konsolArmProfile').value||'npi100',levels:Math.max(1,num('konsolLevels',4)),doubleSided:sideKey()==='double',setPlan:plan,angle:num('konsolAngle',35)}}
  function updateSummary(s){var summary=el('konsolSetSummary');if(summary)summary.innerHTML='<b>ÇAPRAZ SET DAĞILIMI</b>'+s.uprightCount+' ayak → '+planText(s.setPlan)+'<br><small>'+Math.round(s.height).toLocaleString('tr-TR')+' mm ayak boyunda · toplam '+s.setPlan.length+' set · '+s.setPlan.reduce(function(a,b){return a+b},0)+' ayak kapsanıyor.</small>';if(el('konsolAngleValue'))el('konsolAngleValue').value=Math.round(s.angle)}
...
for(const required of ['data-rafex-konsol-v2="1"','data-rafex-krs-native="v9"','/konsol-viewer.js?v=konsol-krs-v12','window.rafexMountKonsolViewer=mount','SSI SCHÄFER KRS KATALOG SEÇİMİ','En üst kol kotu / KRS H','Kat arası mesafe (mm)','Kattaki ağırlık (kg)','6000:{600:{240:7590,270:11420}','1250:{80:505,100:885,120:1420,140:2110}','AORDER=[80,100,120,140]'])if(!html.includes(required))throw new Error('Konsol v12 doğrulaması eksik: '+required);
