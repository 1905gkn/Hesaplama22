...
<script data-rafex-runtime-authority="v2">(function(){
  ...
  var scheduled=0,lastKey='',projectName='',viewerRequests=new Map();
  ...
  function ensureProject(picker){var wrap=document.querySelector('.rafex-common-project-name-wrap');if(!wrap){wrap=document.createElement('section');wrap.className='card rafex-common-project-name-wrap';wrap.innerHTML='<label class="rafex-common-project-name-field"><span>Proje Adı</span><input id="rafexCommonProjectName" type="text" placeholder="Ortak proje adını yaz" autocomplete="off"></label>';}if(wrap.hidden)wrap.hidden=false;if(wrap.hasAttribute('hidden'))wrap.removeAttribute('hidden');if(wrap.getAttribute('data-rafex-authority-project-name')!=='v2')wrap.setAttribute('data-rafex-authority-project-name','v2');if(picker.nextElementSibling!==wrap)picker.insertAdjacentElement('afterend',wrap);var input=wrap.querySelector('#rafexCommonProjectName');if(input){if(!projectName&&input.value)projectName=input.value;if(projectName&&input.value!==projectName)input.value=projectName;}return wrap;}
  ...
  function commit(){scheduled=0;var page=document.getElementById('page');if(!page)return;if(!isCommon(page)){page.removeAttribute('data-rafex-authority-mode');page.removeAttribute('data-rafex-authority-system');lastKey='';return}var picker=document.getElementById('rafexUnifiedSystemPicker');ensureProject(picker);var system=selectedSystem(page)||'b2b';page.setAttribute('data-rafex-authority-mode','common');page.setAttribute('data-rafex-authority-system',system);page.setAttribute('data-rafex-common-active','1');page.setAttribute('data-rafex-common-system',system);var key='common:'+system;if(key!==lastKey){lastKey=key;requestViewer(system);window.dispatchEvent(new CustomEvent('rafex-authority-state',{detail:{mode:'common',system:system}}))}}
