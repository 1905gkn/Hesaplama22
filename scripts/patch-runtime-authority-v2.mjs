...
#page[data-rafex-authority-mode="common"] #rafexCommonProjectName{display:block!important;width:100%!important;height:32px!important;min-height:32px!important;box-sizing:border-box!important}
...
<script data-rafex-runtime-authority="v2">(function(){
  ...
  function isCommon(page){return !!(page&&page.classList.contains('rafex-common-independent'));}
  ...
  function ensureProject(picker){var wraps=Array.from(document.querySelectorAll('.rafex-common-project-name-wrap'));if(!projectNode){var oldInput=wraps[0]&&wraps[0].querySelector('#rafexCommonProjectName');projectName=oldInput&&oldInput.value||projectName;projectNode=document.createElement('section');projectNode.className='card rafex-common-project-name-wrap';projectNode.innerHTML='<label class="rafex-common-project-name-field"><span>Proje Adı</span><input id="rafexCommonProjectName" type="text" placeholder="Ortak proje adını yaz" autocomplete="off"></label>';projectNode.querySelector('#rafexCommonProjectName').addEventListener('input',function(event){projectName=event.target.value;event.stopPropagation();schedule()},true);}wraps.forEach(function(node){if(node!==projectNode)node.remove()});var wrap=projectNode;if(wrap.hidden)wrap.hidden=false;if(wrap.hasAttribute('hidden'))wrap.removeAttribute('hidden');if(wrap.getAttribute('data-rafex-authority-project-name')!=='v2')wrap.setAttribute('data-rafex-authority-project-name','v2');if(picker.nextElementSibling!==wrap)picker.insertAdjacentElement('afterend',wrap);var input=wrap.querySelector('#rafexCommonProjectName');if(input&&input.value!==projectName)input.value=projectName;return wrap;}
  ...
  function commit(){scheduled=0;var page=document.getElementById('page');if(!page)return;if(!isCommon(page)){page.removeAttribute('data-rafex-authority-mode');page.removeAttribute('data-rafex-authority-system');lastKey='';return}var picker=ensurePicker(page);if(!picker)return;ensureProject(picker);var system=selectedSystem(page)||'b2b';page.setAttribute('data-rafex-authority-mode','common');page.setAttribute('data-rafex-authority-system',system);page.setAttribute('data-rafex-common-active','1');page.setAttribute('data-rafex-common-system',system);var key='common:'+system;if(key!==lastKey){lastKey=key;requestViewer(system);window.dispatchEvent(new CustomEvent('rafex-authority-state',{detail:{mode:'common',system:system}}))}}
  function schedule(){if(!scheduled)scheduled=requestAnimationFrame(commit)}
  window.addEventListener('beforeinput',function(event){var target=event.target;if(!target||target.id!=='rafexCommonProjectName')return;var start=target.selectionStart==null?target.value.length:target.selectionStart,end=target.selectionEnd==null?start:target.selectionEnd,next=target.value;if(event.inputType&&event.inputType.indexOf('delete')===0){if(start===end&&start>0)start-=1;next=next.slice(0,start)+next.slice(end)}else{var data=event.data==null?'':event.data;next=next.slice(0,start)+data+next.slice(end);start+=data.length}event.preventDefault();event.stopImmediatePropagation();projectName=next;target.value=next;try{target.setSelectionRange(start,start)}catch(_){}schedule()},true);
  window.addEventListener('input',function(event){if(event.target&&event.target.id==='rafexCommonProjectName'){projectName=event.target.value;event.stopPropagation();schedule()}},true);
  document.addEventListener('input',function(event){if(event.target&&event.target.id==='rafexCommonProjectName')projectName=event.target.value},true);
